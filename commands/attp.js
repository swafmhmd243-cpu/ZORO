...
async function attpCommand(sock, chatId, message) {
    ...
    if (!text) {
        await sock.sendMessage(chatId, { text: 'يرجى تقديم نص بعد الأمر .attp.' }, { quoted: message });
        return;
    }

    try {
        const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);
        const webpPath = await writeExifVid(mp4Buffer, { packname: 'ZORO Bot' });
        const webpBuffer = fs.readFileSync(webpPath);
        try { fs.unlinkSync(webpPath) } catch (_) {}
        await sock.sendMessage(chatId, { sticker: webpBuffer }, { quoted: message });
    } catch (error) {
        console.error('خطأ في إنشاء الملصق محلياً:', error);
        await sock.sendMessage(chatId, { text: 'فشل في إنشاء الملصق محلياً.' }, { quoted: message });
    }
}
...
function renderTextToPngWithFfmpeg(text) {
    return new Promise((resolve, reject) => {
        ...
        ff.on('close', code => {
            ...
            reject(new Error(Buffer.concat(errors).toString() || `ffmpeg خرج بالرمز ${code}`));
        });
    });
}
...
function renderBlinkingVideoWithFfmpeg(text) {
    return new Promise((resolve, reject) => {
        ...
        // طول دورة الوميض (بالثواني) وتأخر سريع ~0.1 ثانية لكل لون
        const cycle = 0.3;
