...
// ✅ Auto Reaction على كل الرسائل إذا كان مفعل
try {
    const { handleAutoReactionOnAllMessages } = require('./lib/reactions');
    await handleAutoReactionOnAllMessages(sock, message);
} catch (err) {
    console.error('خطأ في التفاعل التلقائي:', err);
}
